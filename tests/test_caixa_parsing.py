...
async def test_parse_ofertas_from_static_html():
    ...
    async with async_playwright() as p:
        ...
        ofertas = await caixa._parse_ofertas_from_page(page)
        ...
        assert first.avaliacao == "Valor de avaliação: R$ 190.000,00"
        assert first.valor == "Valor mínimo de venda: R$ 114.838,57 ( desconto de 39,56%)"
        assert first.descricao.startswith("Apartamento - 95,08 m2, 2 quarto(s), 1 vaga(s) na garagem - Venda Direta Online")
        assert first.numero == "878771156863-8"
        def _norm(s: str) -> str:
            import re as _re
